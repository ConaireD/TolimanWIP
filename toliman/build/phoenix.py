"""md
## Overview
Now, you are probably wondering what "phoenix" is? It is a shorthand name 
for a set of stellar spectral models generated by `pysynphot`/`synphot`. 
Most of the required datafiles are required for this model, even though 
we studiously avoid downloading all but the essential files. When setting 
the forwards model up, you will notice that `pysynphot` reports a lot of 
missing files. That is because we only install the files that are required 
for the phoenix model and no others.

We have designed the code to save the spectra in a `.csv` of your choosing,
since it is something that it unlikely (for the mean time) to be learned. 
This may change with the addition of the sidelobes. However, if you wish to 
simply generate the `.csv` and then delete the datafiles required by 
`pysynphot` you will save some space and loose none of the forwards models 
functionality. 
