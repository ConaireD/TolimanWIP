...
## 26/03/23: Working to add sidelobes and functionality to this class to allow this
    - Conaire
...
class TolimanOptics(dl.Optics, collections.CollectionInterface):
    ...
    def __init__(
        ...
        pupil_diameter: float = float(os.environ["TOLIMAN_PRIMARY_APERTURE_DIAMETER"]),
        grating_amplitude: float = None,
        pixel_size: float = float(os.environ['TOLIMAN_DETECTOR_PIXEL_SIZE'])
    ) -> object:
        ...
        pupil_diameter: float = TOLIMAN_PRIMARY_APERTURE_DIAMETER
            The diameter of the Toliman pupil in metres.
        grating_amplitude: float = None
            The amplitude of the pupil grating
        pixel_size: float = TOLIMAN_DETECTOR_PIXEL_SIZE
            The size of the pixels in radians on the detector
            
        --------------------------------------------------------
                              Known Bugs
        --------------------------------------------------------
        Bug #1:
            An error is thrown if pixels_on_detector != pixels_in_pupil
            
        Bug #2:
            When a grating amplitude is given the mask is bugged
            
        Bug #3:
            Changing the mask file doesn't change it here...
        ...
        try:
            ...
            # Hacky Solution #
            ##################
            loaded_mask = np.round((loaded_mask*1e7)/np.pi)*1e-7*np.pi
        ...
        loaded_mask: float = np.load(path_to_mask)
        loaded_mask = np.round((loaded_mask*1e7)/np.pi)*1e-7*np.pi # hacky

        if grating_amplitude == None:
            mask: float = loaded_mask
            #np.save('test-mask-ignore', mask)

        else:
            
            diffraction_angle_arcsec = 0.75*dl.utils.radians_to_arcseconds(pixel_size)*np.sqrt(2)*(pixels_on_detector/2)

            diffraction_angle = dl.utils.arcseconds_to_radians(diffraction_angle_arcsec) # Approximate
            period = float(os.environ['FILTER_MAX_WAVELENGTH'])/np.sin(diffraction_angle)
            grating_sampling = pixels_on_detector / (2*pupil_diameter/period)
            #print("Grating is sampled as {:.2f}x Nyquist".format(grating_sampling))

            # add grating changes
            G, AG = self.generate_sinusoids(pupil_diameter, pixels_on_detector, period, grating_amplitude)
            mask: float = self.add_grating_to_mask(loaded_mask, G, AG)
            #np.save('test-mask-ignore', mask)
    ...
    def generate_sinusoids(self, aperture_size, N, period, ampl):
        '''
        Generates sinusoids for the grating.
        
        Returns
        --------
        grating: numpy.ndarray
            A 2d array of the same size as the pupil containing one
            of the two gratings.
        anti_grating: numpy.ndarray
            A 2d array as above containing the anti grating.
        '''
        coords = dl.utils.get_pixel_coordinates(N, aperture_size/N)

        R, PHI = dl.utils.cartesian_to_polar(coords)
        PHI += np.pi/4 # Rotate by 45 degrees
        polar_coords = np.array([R, PHI])
        X, Y = dl.utils.polar_to_cartesian(polar_coords)

        # Define the sine argument value to get the correct period
        B = 2*np.pi / period

        # Creating gratings
        grating =      (ampl*np.cos(B*Y) +          ampl*np.cos(B*X)         )/4
        anti_grating = (ampl*np.cos(B*Y  + np.pi) + ampl*np.cos(B*X + np.pi))/4
        ...
        return grating, anti_grating
    
    def add_grating_to_mask(self, loaded_mask, grating, anti_grating):
        """
        Adds the grating to the mask.
        
        Returns
        -------
        mask_with_gratings: numpy.ndarray
            The new pupil mask.
        """
        mask = np.round((loaded_mask/np.pi)*1e7, decimals=0)

        bool_mask = mask.astype(bool)
        anti_mask = np.invert(bool_mask)

        grating_in = (bool_mask * grating) + (anti_mask * anti_grating)
        OPD_mask = loaded_mask * 0.25 * (float(os.environ["FILTER_MIN_WAVELENGTH"]) + float(os.environ["FILTER_MAX_WAVELENGTH"]))
        mask_with_grating = OPD_mask + grating_in + loaded_mask
        # note: not sure why we need to add loaded mask...
        return mask_with_grating
