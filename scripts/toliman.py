...
width_of_pupil: float = .125
width_of_obscuration: float = .025
number_of_pixels: int = 128
pixel_scale: float = 2. * width_of_pupil / number_of_pixels

toliman_aperture = dl.CompoundAperture([
    dl.AnnularAperture(width_of_pupil, width_of_obscuration),
    dl.UniformSpider(number_of_struts, width_of_struts)
])

tol_aper_arr: float = toliman_aperture.get_aperture(number_of_pixels, 2 * width_of_pupil) 

fig = plt.figure()
axes = plt.axes()
cmap = axes.imshow(tol_aper_arr)
fig.colorbar(cmap, ax=axes)
plt.show()

annular_aperture = dl.AnnularAperture(width_of_pupil, width_of_obscuration)
