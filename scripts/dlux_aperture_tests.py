import matplotlib.pyplot as plt
import matplotlib as mpl
import jax.numpy as np
import dLux as dl

mpl.rcParams["text.usetex"] = True
mpl.rcParams["image.cmap"] = "inferno"
mpl.rcParams["axes.spines.top"] = False
mpl.rcParams["axes.spines.left"] = False
mpl.rcParams["axes.spines.right"] = False
mpl.rcParams["axes.spines.bottom"] = False


coordinates = dl.utils.get_pixel_coordinates(24, 2. / 24.)

# My philosphy with the following trest cases is that each feature should
# only be tested once. In other words, I do not need to test every combination 
# of values just enough that any edge cases/branches are tested. Otherwise 
# there would be a vast number of outputs to check. As with all unit testing 
# the idea that only one thing is tested at a time is also used.
occ_soft_circ = dl.CircularAperture(0., 0., 1., occulting=True, softening=True)
occ_circ = dl.CircularAperture(0., 0., 1., occulting=True, softening=False)
soft_circ = dl.CircularAperture(0., 0., 1., occulting=False, softening=True)
circ = dl.CircularAperture(0., 0., 1., occulting=False, softening=False)
pos_y_circ = dl.CircularAperture(0., 1., 1., occulting=True, softening=True)
pos_x_circ = dl.CircularAperture(1., 0., 1., occulting=True, softening=True)
neg_y_circ = dl.CircularAperture(0., -1., 1., occulting=True, softening=True)
neg_x_circ = dl.CircularAperture(-1., 0., 1., occulting=True, softening=True)

fig, axes = plt.subplots(2, 4, figsize=(4*4, 2*3))
fig.suptitle("Circular Aperture Tests")
axes[0][0].set_title("Occulting, Soft Edged")
_map = axes[0][0].imshow(occ_soft_circ._aperture(coordinates))
axes[0][0].set_xticks([])
axes[0][0].set_yticks([])
fig.colorbar(_map)
axes[0][1].set_title("Occulting, Hard Edged")
_map = axes[0][1].imshow(occ_circ._aperture(coordinates))
axes[0][1].set_xticks([])
axes[0][1].set_yticks([])
fig.colorbar(_map)
axes[0][2].set_title("Not Occulting, Soft Edged")
_map = axes[0][2].imshow(soft_circ._aperture(coordinates))
axes[0][2].set_xticks([])
axes[0][2].set_yticks([])
fig.colorbar(_map)
axes[0][3].set_title("Not Occulting, Hard Edged")
_map = axes[0][3].imshow(circ._aperture(coordinates))
axes[0][3].set_xticks([])
axes[0][3].set_yticks([])
fig.colorbar(_map)
axes[1][0].set_title("Positive $y$ Translation (Occ., Soft.)")
_map = axes[1][0].imshow(pos_y_circ._aperture(coordinates))
axes[1][0].set_xticks([])
axes[1][0].set_yticks([])
fig.colorbar(_map)
axes[1][1].set_title("Positive $x$ Translation (Occ., Soft.)")
_map = axes[1][1].imshow(pos_x_circ._aperture(coordinates))
axes[1][1].set_xticks([])
axes[1][1].set_yticks([])
fig.colorbar(_map)
axes[1][2].set_title("Negative $y$ Translation (Occ., Soft.)")
_map = axes[1][2].imshow(neg_y_circ._aperture(coordinates))
axes[1][2].set_xticks([])
axes[1][2].set_yticks([])
fig.colorbar(_map)
axes[1][3].set_title("Negative $x$ Translation (Occ., Soft.)")
_map = axes[1][3].imshow(neg_x_circ._aperture(coordinates))
axes[1][3].set_xticks([])
axes[1][3].set_yticks([])
fig.colorbar(_map)
plt.show()

# Annular Aperture Tests
occ_soft_annular = dl.AnnularAperture(0., 0., 1., .5, occulting=True, softening=True)
occ_annular = dl.AnnularAperture(0., 0., 1., .5, occulting=True, softening=False)
soft_annular = dl.AnnularAperture(0., 0., 1., .5, occulting=False, softening=True)
annular = dl.AnnularAperture(0., 0., 1., .5, occulting=False, softening=False)
pos_y_annular = dl.AnnularAperture(0., 1., 1., .5, occulting=True, softening=True)
pos_x_annular = dl.AnnularAperture(1., 0., 1., .5, occulting=True, softening=True)
neg_y_annular = dl.AnnularAperture(0., -1., 1., .5, occulting=True, softening=True)
neg_x_annular = dl.AnnularAperture(-1., 0., 1., .5, occulting=True, softening=True)

fig, axes = plt.subplots(2, 4, figsize=(4*4, 2*3))
fig.suptitle("Annular Aperture Tests")
axes[0][0].set_title("Occulting, Soft Edged")
_map = axes[0][0].imshow(occ_soft_annular._aperture(coordinates))
axes[0][0].set_xticks([])
axes[0][0].set_yticks([])
fig.colorbar(_map)
axes[0][1].set_title("Occulting, Hard Edged")
_map = axes[0][1].imshow(occ_annular._aperture(coordinates))
axes[0][1].set_xticks([])
axes[0][1].set_yticks([])
fig.colorbar(_map)
axes[0][2].set_title("Not Occulting, Soft Edged")
_map = axes[0][2].imshow(soft_annular._aperture(coordinates))
axes[0][2].set_xticks([])
axes[0][2].set_yticks([])
fig.colorbar(_map)
axes[0][3].set_title("Not Occulting, Hard Edged")
_map = axes[0][3].imshow(annular._aperture(coordinates))
axes[0][3].set_xticks([])
axes[0][3].set_yticks([])
fig.colorbar(_map)
axes[1][0].set_title("Positive $y$ Translation (Occ., Soft.)")
_map = axes[1][0].imshow(pos_y_annular._aperture(coordinates))
axes[1][0].set_xticks([])
axes[1][0].set_yticks([])
fig.colorbar(_map)
axes[1][1].set_title("Positive $x$ Translation (Occ., Soft.)")
_map = axes[1][1].imshow(pos_x_annular._aperture(coordinates))
axes[1][1].set_xticks([])
axes[1][1].set_yticks([])
fig.colorbar(_map)
axes[1][2].set_title("Negative $y$ Translation (Occ., Soft.)")
_map = axes[1][2].imshow(neg_y_annular._aperture(coordinates))
axes[1][2].set_xticks([])
axes[1][2].set_yticks([])
fig.colorbar(_map)
axes[1][3].set_title("Negative $x$ Translation (Occ., Soft.)")
_map = axes[1][3].imshow(neg_x_annular._aperture(coordinates))
axes[1][3].set_xticks([])
axes[1][3].set_yticks([])
fig.colorbar(_map)
plt.show()

# Rectagular Aperture Tests
occ_soft_rect = dl.RectangularAperture(0., 0., 0., 1., .5, occulting=True, softening=True)
occ_rect = dl.RectangularAperture(0., 0., 0., 1., .5, occulting=True, softening=False)
soft_rect = dl.RectangularAperture(0., 0., 0., 1., .5, occulting=False, softening=True)
rect = dl.RectangularAperture(0., 0., 0., 1., .5, occulting=False, softening=False)
pos_y_rect = dl.RectangularAperture(0., 1., 0., 1., .5, occulting=True, softening=True)
pos_x_rect = dl.RectangularAperture(1., 0., 0., 1., .5, occulting=True, softening=True)
neg_y_rect = dl.RectangularAperture(0., -1., 0., 1., .5, occulting=True, softening=True)
neg_x_rect = dl.RectangularAperture(-1., 0., 0., 1., .5, occulting=True, softening=True)
pos_rot_rect = dl.RectangularAperture(0., 0., np.pi / 4., 1., .5, occulting=True, softening=True)
neg_rot_rect = dl.RectangularAperture(0., 0., -np.pi / 4., 1., .5, occulting=True, softening=True)

fig, axes = plt.subplots(2, 5, figsize=(5*4, 2*3))
fig.suptitle("Rectangular Aperture Tests")
axes[0][0].set_title("Occulting, Soft Edged")
_map = axes[0][0].imshow(occ_soft_rect._aperture(coordinates))
axes[0][0].set_xticks([])
axes[0][0].set_yticks([])
fig.colorbar(_map)
axes[0][1].set_title("Occulting, Hard Edged")
_map = axes[0][1].imshow(occ_rect._aperture(coordinates))
axes[0][1].set_xticks([])
axes[0][1].set_yticks([])
fig.colorbar(_map)
axes[0][2].set_title("Not Occulting, Soft Edged")
_map = axes[0][2].imshow(soft_rect._aperture(coordinates))
axes[0][2].set_xticks([])
axes[0][2].set_yticks([])
fig.colorbar(_map)
axes[0][3].set_title("Not Occulting, Hard Edged")
_map = axes[0][3].imshow(rect._aperture(coordinates))
axes[0][3].set_xticks([])
axes[0][3].set_yticks([])
fig.colorbar(_map)
axes[0][4].set_title("Positive $y$ Translation (Occ., Soft.)")
_map = axes[0][4].imshow(pos_y_rect._aperture(coordinates))
axes[0][4].set_xticks([])
axes[0][4].set_yticks([])
fig.colorbar(_map)
axes[1][0].set_title("Positive $x$ Translation (Occ., Soft.)")
_map = axes[1][0].imshow(pos_x_rect._aperture(coordinates))
axes[1][0].set_xticks([])
axes[1][0].set_yticks([])
fig.colorbar(_map)
axes[1][1].set_title("Negative $y$ Translation (Occ., Soft.)")
_map = axes[1][1].imshow(neg_y_rect._aperture(coordinates))
axes[1][1].set_xticks([])
axes[1][1].set_yticks([])
fig.colorbar(_map)
axes[1][2].set_title("Negative $x$ Translation (Occ., Soft.)")
_map = axes[1][2].imshow(neg_x_rect._aperture(coordinates))
axes[1][2].set_xticks([])
axes[1][2].set_yticks([])
fig.colorbar(_map)
axes[1][3].set_title("Positive Rotation (occ. Soft.)")
_map = axes[1][3].imshow(pos_rot_rect._aperture(coordinates))
axes[1][3].set_xticks([])
axes[1][3].set_yticks([])
fig.colorbar(_map, ax=axes[1][3])
axes[1][4].set_title("Negative Rotation (occ. Soft.)")
_map = axes[1][4].imshow(neg_rot_rect._aperture(coordinates))
axes[1][4].set_xticks([])
axes[1][4].set_yticks([])
fig.colorbar(_map, ax=axes[1][4])
plt.show()

# Square Aperture Tests
occ_soft_sq = dl.SquareAperture(0., 0., 0., 1., occulting=True, softening=True)
occ_sq = dl.SquareAperture(0., 0., 0., 1., occulting=True, softening=False)
soft_sq = dl.SquareAperture(0., 0., 0., 1., occulting=False, softening=True)
sq = dl.SquareAperture(0., 0., 0., 1., occulting=False, softening=False)
pos_y_sq = dl.SquareAperture(0., 1., 0., 1., occulting=True, softening=True)
pos_x_sq = dl.SquareAperture(1., 0., 0., 1., occulting=True, softening=True)
neg_y_sq = dl.SquareAperture(0., -1., 0., 1., occulting=True, softening=True)
neg_x_sq = dl.SquareAperture(-1., 0., 0., 1., occulting=True, softening=True)
pos_rot_sq = dl.SquareAperture(0., 0., np.pi / 4., 1., occulting=True, softening=True)
neg_rot_sq = dl.SquareAperture(0., 0., -np.pi / 4., 1., occulting=True, softening=True)

fig, axes = plt.subplots(2, 5, figsize=(5*4, 2*3))
fig.suptitle("Square Aperture Tests")
axes[0][0].set_title("Occulting, Soft Edged")
_map = axes[0][0].imshow(occ_soft_sq._aperture(coordinates))
axes[0][0].set_xticks([])
axes[0][0].set_yticks([])
fig.colorbar(_map)
axes[0][1].set_title("Occulting, Hard Edged")
_map = axes[0][1].imshow(occ_sq._aperture(coordinates))
axes[0][1].set_xticks([])
axes[0][1].set_yticks([])
fig.colorbar(_map)
axes[0][2].set_title("Not Occulting, Soft Edged")
_map = axes[0][2].imshow(soft_sq._aperture(coordinates))
axes[0][2].set_xticks([])
axes[0][2].set_yticks([])
fig.colorbar(_map)
axes[0][3].set_title("Not Occulting, Hard Edged")
_map = axes[0][3].imshow(sq._aperture(coordinates))
axes[0][3].set_xticks([])
axes[0][3].set_yticks([])
fig.colorbar(_map)
axes[0][4].set_title("Positive $y$ Translation (Occ., Soft.)")
_map = axes[0][4].imshow(pos_y_sq._aperture(coordinates))
axes[0][4].set_xticks([])
axes[0][4].set_yticks([])
fig.colorbar(_map)
axes[1][0].set_title("Positive $x$ Translation (Occ., Soft.)")
_map = axes[1][0].imshow(pos_x_sq._aperture(coordinates))
axes[1][0].set_xticks([])
axes[1][0].set_yticks([])
fig.colorbar(_map)
axes[1][1].set_title("Negative $y$ Translation (Occ., Soft.)")
_map = axes[1][1].imshow(neg_y_sq._aperture(coordinates))
axes[1][1].set_xticks([])
axes[1][1].set_yticks([])
fig.colorbar(_map)
axes[1][2].set_title("Negative $x$ Translation (Occ., Soft.)")
_map = axes[1][2].imshow(neg_x_sq._aperture(coordinates))
axes[1][2].set_xticks([])
axes[1][2].set_yticks([])
fig.colorbar(_map)
axes[1][3].set_title("Positive Rotation (occ. Soft.)")
_map = axes[1][3].imshow(pos_rot_sq._aperture(coordinates))
axes[1][3].set_xticks([])
axes[1][3].set_yticks([])
fig.colorbar(_map, ax=axes[1][3])
axes[1][4].set_title("Negative Rotation (occ. Soft.)")
_map = axes[1][4].imshow(neg_rot_sq._aperture(coordinates))
axes[1][4].set_xticks([])
axes[1][4].set_yticks([])
fig.colorbar(_map, ax=axes[1][4])
plt.show()

# Uniform Spider Testing
even_soft_unif_spider = dl.UniformSpider(0., 0., 4., .1, 0., softening=True)
odd_soft_unif_spider = dl.UniformSpider(0., 0., 3., .1, 0., softening=True)

fig, axes = plt.subplots(2, 2, figsize=(2*4, 1*3))
axes[0][0].imshow(even_soft_unif_spider._aperture(coordinates))
plt.show()
